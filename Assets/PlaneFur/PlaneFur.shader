    ...
    SubShader
    {
        ...
        Pass 
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                col.a = col.r;
                fixed AO = col.b;
                col.rgb = lerp(_BaseColor.rgb, _BaseColor.rgb * AO,_AOIntensity);

                float3 viewDir = normalize(_WorldSpaceCameraPos - i.worldPos.xyz);
                float3 curWorldPos = i.worldPos + viewDir * sqrt(max(0.25 - dot(i.centerOffset.xy, i.centerOffset.xy), 0.0));
                float3 worldNormal = normalize(curWorldPos - i.OriginalPos);

                float3 lightDir = normalize(UnityWorldSpaceLightDir(curWorldPos));
                float ndl = saturate(dot(worldNormal,lightDir)) * 0.5 + 0.5;
                col.rgb *= ndl;
                col.a = 1.0;
