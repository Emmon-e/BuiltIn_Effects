Shader "Unlit/Clouds"
{
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			Texture2D<float4> _WeatherTex;
			SamplerState sampler_WeatherTex;
            ...
            float2 RayBoxDist(float3 boundsMin,float3 boundsMax,float3 rayOrigin, float3 rayDir)
            {
               ...
               float3 t0 = (boundsMin - rayOrigin) / rayDir;  // 相当求boundsMin - rayOrigin是多少倍的rayDir；
                float3 t1 = (boundsMax - rayOrigin) / rayDir;
                float3 tmin = min(t0,t1); 
                float3 tmax = max(t0,t1);
                float distA = max(max(tmin.x,tmin.y),tmin.z); // 找到最小的距离分量
                float distB = min(min(tmax.x,tmax.y),tmax.z); //找到最大的距离分量
			...
			float SampleDensity(float3 rayPos)
			{
				...
				//float2 weatherUV = (size.xz * 0.5 + (rayPos.xz - boundsCenter.xz)) / max(size.x, size.z);
				//float4 weather = _WeatherTex.SampleLevel(sampler_WeatherTex, weatherUV,0);
