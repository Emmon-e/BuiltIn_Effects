// 
	...
	half4 SkinBRDF(half3 diffuseColor, half3 specularColor,half3 emission, half oneMinusReflectivity, half smoothness, float3 normal, float3 viewDir, UnityLight light, UnityIndirect gi)
	{
	...
	#define UNITY_HANDLE_CORRECTLY_NEGATIVE_NDOTV 0//默认情况下走方法2，假如要走方法1需要注释此行
		...
		float ndv = saturate(dot(normal, viewDir)); // TODO: this saturate should no be necessary here
	#else//方法2，默认走此方法
		half ndv = abs(dot(normal, viewDir));    // This abs allow to limit artifact
	#endif
		...
		half diffuseTerm = DisneyDiffuse(ndv, ndl, hdl, roughness) * ndl;  //迪士尼的漫反射计算没有除以PI， 正常是要除的，这里是因为要兼容非重要灯光及旧效果
		...
		half3 diffuseResult = diffuseColor *  (gi.diffuse + light.color * diffuseTerm ) + emission;
	...
	inline half4 LightingSkinShadingMode(SurfaceOutputStandard s, float3 viewDir, UnityGI gi)
	{
		s.Normal = normalize(s.Normal);
	
		half oneMinusReflectivity;// 漫反射系数（Albedo中参与漫反射的比例）
		half3 specularColor;
		s.Albedo = DiffuseAndSpecularFromMetallic(s.Albedo, s.Metallic, specularColor, oneMinusReflectivity); // UnityStandardUtils.cginc 
	
		half outAlpha;
		s.Albedo = PreMultiplyAlpha(s.Albedo, s.Alpha, oneMinusReflectivity, outAlpha);// 根据金属度处理Alpha混和
		half4 col = SkinBRDF(s.Albedo, specularColor, s.Emission,oneMinusReflectivity, s.Smoothness, s.Normal, viewDir, gi.light, gi.indirect);
		col.a = outAlpha;
		return col;
